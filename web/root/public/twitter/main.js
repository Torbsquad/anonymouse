...
async function main() {
  ...
  for (let data of e.data) {
    ...
    image.src =
      "data:image/png;base64," + _arrayBufferToBase64(data.picture.data);
    ...
    let entryContent = `<a href="https://twitter.com/${data.screen_name}">${
      data.name
    }</a> <a class="sidenote" href="https://twitter.com/${data.screen_name}">@${
      data.screen_name
    }</a>
        ...
        id: ${data.follower_id}
        still following: <span style="color:${
          data.still_following ? "green" : "grey"
        }">${data.still_following}</span>
      `.replace(/\n/g, "<br />");
